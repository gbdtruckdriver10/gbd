...
export default function StaffMessagesPage() {
  ...
  const [parents, setRecipients] = useState<Recipient[]>([]);
  ...
  useEffect(() => {
    ...
    ]).then(([msgs, anns, parentList, adminList, cfoList]) => {
      setMessages(msgs);
      setAnnouncements(
        (anns as Announcement[]).filter((a) => a.audience === "staff" || a.audience === "all")
      );
      setRecipients([...parentList, ...adminList, ...cfoList]);
      setLoading(false);
    });
  }, [user?.id]);
  ...
  const handleSend = async () => {
    ...
    if (res.ok) {
      ...
      const selected = parents.find((p) => p.user_id === Number(receiverId));
      setMessages((prev) => [
        {
          ...newMsg,
          subject: subject.trim() || "(no subject)",
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Communicate with parents and admin</p>
      </div>
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          <div>
            ...
            <Input
              className="mt-1"
              placeholder="e.g. Update on today's session"
              value={subject}
      ...
      {/* Announcements */}
      <div>
        <h2 className="text-2xl font-bold text-[#002040] mb-4">Staff Announcements</h2>
