...
export default function AdminMessagesPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([msgs, anns, parentList, staffList, cfoList]) => {
      setMessages(msgs);
      setAnnouncements((anns as Announcement[]).slice(0, 5));
      setRecipients([...parentList, ...staffList, ...cfoList]);
      setLoading(false);
    });
  }, [user?.id]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Communicate with staff, parents, and the CFO</p>
      </div>
